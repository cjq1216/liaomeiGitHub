...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if (arr.profile === 'user does not exist!') {
                window.location.href = "/liaomei/liaomeiServer/Login/login.php"
            }
            var pro = arr.profile;
            ...
            var socket = io.connect('http://cjunqi.com:3000');
            setTimeout(function(){
            ...
            $('#sureSearch').click(function(){
                ...
                if (searchId === ''){
                    showOuterMessage('#searcherForm', false, '请输入对方账户');
                } else {
                    socket.emit('searchFriend',{searchId:searchId, my_id:user_id});
                }
            })
            socket.on('searchResult',function(searchFriendDetail){
                if ( !searchFriendDetail ) {
                    showOuterMessage('#searcherForm', false, '没有找到该用户');
                }
                else {
                    $('#searcherForm').hide();
                    $('#searcherResult').show();
            ...
            socket.on('newHi',function(newFriend) {
            ...
            socket.on('acceptedAdd',function(newFriend) {
                friend.push(newFriend);
                setFriends(friend);
                setChatList(user_head_portrait, user_name, friend);
                setChatDetailSwitch(friend, user_head_portrait);
            ...
            $('#acceptFriend').click(function(){
                ...
                socket.emit('addConfirm', addMessage);
            })
            $('#refuseFriend').click(function(){
                ...
                console.log("refuseConfirm");
                socket.emit('refuseConfirm', refuseMessage);
            })
            ...
            socket.on('updateNickSucess',function(msg) {
                for(var i=0;i<friend.length;i++) {
            ...
            $('#deleteFriendButton').click(function() {
                socket.emit('delFriend',{
                    my_id:user_id,
                    friend_id:$('#friendPro-id').html(),
                });
            })
            socket.on('delSuccess',function(msg) {
                for(var i=0;i<friend.length;i++) {
                    if(friend[i].friend_id == msg.friend_id) {
                        console.log("Delete");
                        friend.remove(i);
            ...
            socket.on('updateDataSuccess',function(msg) {
    ...
    $('#unlogin').click(function(){
      self.location.href='/liaomei/liaomeiServer/Login/login.php';
    })
