...
io.sockets.on('connection', function(socket){
            ...
            client.query(sql2, function(err2, rows){
                ...
                for (var o in group ) {
                    if ( group[o].user_id === msg.receiver_id) {
                        msg.pm_state = 1;
                        group[o].socket.emit('receiveMessage', msg);
                        break;
                    }
                }
    ...
    socket.on('searchFriend', function(msg){
        ...
        client.query(sql1, function(err1, rows1){
            ...
            client.query(sql2, function(err2, rows2) {
                ...
                socket.emit('searchResult',rows1[0]);
                client.end();
            })
        });
    })
                    ...
                    if ( group[o].user_id === msg.receiver_id) {
                        client.query(sql3,function(err3,row1) {
                            ...
                            client.query(sql4,function(err4,row2) {
                                ...
                                group[o].socket.emit('newHi', newFriend);
                            });
                        });
                        break;
                    }
    ...
    socket.on('addConfirm',function(msg){
        var client = mysql.createConnection(sqlCon)
        client.connect();
        console.log(msg);
                ...
                for (var o in group ) {
                    if ( group[o].user_id === msg.my_id) {
                        client.query(sql3, function(err3,row) {
                            ...
                            client.end();
                            console.log(newFriend);
                            group[o].socket.emit('acceptedAdd', newFriend);
                        })
                        break;
                    }
                }
    ...
    socket.on('refuseConfirm',function(msg){
        var client = mysql.createConnection(sqlCon)
        client.connect();
        console.log(msg);
    ...
    socket.on('updateNickName',function(msg) {
        ...
        client.query(sql, function(err,res){
            ...
            socket.emit('updateNickSucess',msg);
            client.end();
        });
    });
    socket.on('updateData',function(msg) {
        ...
        client.query(sql, function(err,res){
            if(err) console.log(err);
            socket.emit('updateDataSuccess',msg);
            client.end();
        });
    });
                    ...
                    client.query(sql4, function(err4, res4) {
                        ...
                        for (var o in group ) {
                            if ( group[o].user_id === msg.friend_id) {
                                ...
                                group[o].socket.emit('delSuccess', {
                                    friend_id:msg.my_id
                                });
                                break;
                            }
                        }
                        socket.emit('delSuccess',{
